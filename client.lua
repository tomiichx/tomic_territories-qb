...
local QBCore, PlayerData, territoriesClient, Blips, iBlips, progress, isTaking = exports['qb-core']:GetCoreObject(), {}, {}, {}, {}, 0, false
...
CreateThread(function()
    QBCore.Functions.TriggerCallback('tomic_territories:getTerritories', function(territories) 
...
AddEventHandler('tomic_territories:createTerritory', function()
    ...
    if input then
        if input[1] ~= nil and tonumber(input[2]) ~= nil and input[3] ~= nil then
            ...
            QBCore.Functions.Notify('You must fill all fields correctly!')
        end
    else
        QBCore.Functions.Notify('You cancelled the creation of a new territory.')
    end
end)
    ...
    while true do
        ...
        if isLoggedIn then
            for k, v in pairs(territoriesClient) do
                ...
                if PlayerGang and shared.gangs[PlayerGang.name] then
    ...
    if data.data.spawnano == false then
        ...
        if spawnpoint then
            if closestPlayer == -1 or closestPlayerDistance > 3.0 then
                QBCore.Functions.Notify('devTomic | There is no one close. You need one more person to start the mission!')
            else
            ...
            QBCore.Functions.Notify('devTomic | Spawnpoint is not clear!')
        end
    else
        QBCore.Functions.Notify('devTomic | Mission has already been started!')
    end
end)
...
AddEventHandler('tomic_territories:listaItema', function(data)
    ...
    if PlayerGang.name == data.data.currentOwner then
        ...
        QBCore.Functions.Notify('devTomic | You do not own this territory!')
    end
end)
...
AddEventHandler('tomic_territories:letCount', function(selected, datax)
    ...
    if input then
        ...
        if count < 1 then
            return QBCore.Functions.Notify('devTomic | Amount cannot be lower than 1!')
        else
            allInfo = {
    ...
    if territorydata.job ~= territorydata.currentOwner then
        if territorydata.taking == false then
            if territorydata.cooldown == false then
                ...
                QBCore.Functions.Notify('devTomic | This territory was recently captured, or capture was attempted!')
            end
        else
            QBCore.Functions.Notify('devTomic | Someone is already taking the territory!')
        end
    else
        QBCore.Functions.Notify('devTomic | This territory already belongs to you!')
    end
end)
...
AddEventHandler('tomic_territories:openStash', function(x)
    ...
    if distanca < 5.0 then
        if PlayerGang.name == territorydata.currentOwner then
...
AddEventHandler('tomic_territories:captureprogress', function(datakey, data)
    while true do
        ...
        if distance < prekidanje then
            if not isDead then
                if data.isTaking == true then
                    if progress < 60 then
                        ...
                        progress = 0
                        TriggerEvent('tomic_territories:progressBars', 'stop')
                        QBCore.Functions.Notify('devTomic | You have successfully captured '..data.name..'!')
                        TriggerEvent('tomic_territories:progressBars', 'stop')
                        break
                    end
                else
                ...
                TriggerEvent('tomic_territories:progressBars', 'stop')
                QBCore.Functions.Notify('devTomic | You died, the capturing progress has stopped!')
                progress = 0
            ...
            TriggerEvent('tomic_territories:progressBars', 'stop')
            QBCore.Functions.Notify('devTomic | You left the territory, capturing progress has stopped!')
            progress = 0
            TriggerServerEvent('tomic_territories:captureend', lastTerritory)
            lastTerritory = nil
            break
...
AddEventHandler('tomic_territories:trecastrana', function(args)
    QBCore.Functions.TriggerCallback('tomic_territories:povucipoene', function(lista)
        local trTabela = {}
        if territoriesClient ~= nil then
            for i = 1, #lista, 1 do
